defmodule Citadel.TraceBridge do
  ...
  @impl true
  @spec publish_trace(TraceEnvelope.t() | map() | keyword()) :: :ok | {:error, reason_code()}
  def publish_trace(envelope) do
    envelope = TraceEnvelope.new!(envelope)
    adapter().publish_trace(envelope)
  rescue
    ArgumentError -> {:error, :invalid_envelope}
  end

  @impl true
  @spec publish_traces([TraceEnvelope.t() | map() | keyword()]) :: :ok | {:error, reason_code()}
  def publish_traces(envelopes) when is_list(envelopes) do
    normalized =
      Enum.map(envelopes, fn envelope ->
        TraceEnvelope.new!(envelope)
      end)

    adapter().publish_traces(normalized)
  rescue
    ArgumentError -> {:error, :invalid_envelope}
  end
