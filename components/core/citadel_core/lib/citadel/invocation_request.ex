defmodule Citadel.InvocationRequest do
  @moduledoc """
  ...
  defp build!(attrs) do
    attrs = AttrMap.normalize!(attrs, "Citadel.InvocationRequest attrs")

    %__MODULE__{
      schema_version:
        attrs
        |> AttrMap.fetch!(:schema_version, "Citadel.InvocationRequest")
        |> validate_schema_version!(),
      invocation_request_id:
        attrs
        |> AttrMap.fetch!(:invocation_request_id, "Citadel.InvocationRequest")
        |> validate_non_empty_string!(:invocation_request_id),
      request_id:
        attrs
        |> AttrMap.fetch!(:request_id, "Citadel.InvocationRequest")
        |> validate_non_empty_string!(:request_id),
      session_id:
        attrs
        |> AttrMap.fetch!(:session_id, "Citadel.InvocationRequest")
        |> validate_non_empty_string!(:session_id),
      tenant_id:
        attrs
        |> AttrMap.fetch!(:tenant_id, "Citadel.InvocationRequest")
        |> validate_non_empty_string!(:tenant_id),
      trace_id:
        attrs
        |> AttrMap.fetch!(:trace_id, "Citadel.InvocationRequest")
        |> validate_non_empty_string!(:trace_id),
      actor_id:
        attrs
        |> AttrMap.fetch!(:actor_id, "Citadel.InvocationRequest")
        |> validate_non_empty_string!(:actor_id),
      target_id:
        attrs
        |> AttrMap.fetch!(:target_id, "Citadel.InvocationRequest")
        |> validate_non_empty_string!(:target_id),
      target_kind:
        attrs
        |> AttrMap.fetch!(:target_kind, "Citadel.InvocationRequest")
        |> validate_non_empty_string!(:target_kind),
      selected_step_id:
        attrs
        |> AttrMap.fetch!(:selected_step_id, "Citadel.InvocationRequest")
        |> validate_non_empty_string!(:selected_step_id),
      allowed_operations:
        attrs
        |> AttrMap.fetch!(:allowed_operations, "Citadel.InvocationRequest")
        |> validate_allowed_operations!(),
      authority_packet:
        attrs
        |> AttrMap.fetch!(:authority_packet, "Citadel.InvocationRequest")
        |> validate_authority_packet!(),
      boundary_intent:
        attrs
        |> AttrMap.fetch!(:boundary_intent, "Citadel.InvocationRequest")
        |> validate_boundary_intent!(),
      topology_intent:
        attrs
        |> AttrMap.fetch!(:topology_intent, "Citadel.InvocationRequest")
        |> validate_topology_intent!(),
      extensions:
        attrs
        |> AttrMap.fetch!(:extensions, "Citadel.InvocationRequest")
        |> validate_extensions!()
    }
  end
  ...
  defp validate_schema_version!(value) do
    raise ArgumentError,
          "Citadel.InvocationRequest.schema_version must be #{@schema_version}, got: #{inspect(value)}"
  end

  defp validate_non_empty_string!(value, field) when is_binary(value) do
    if String.trim(value) == "" do
      raise ArgumentError, "Citadel.InvocationRequest.#{field} must be a non-empty string"
    end
  ...
  defp validate_non_empty_string!(value, field) do
    raise ArgumentError,
          "Citadel.InvocationRequest.#{field} must be a non-empty string, got: #{inspect(value)}"
  end
  ...
  defp validate_allowed_operations!(value) when is_list(value) do
    normalized =
      Enum.map(value, fn
        ...
        operation ->
          raise ArgumentError,
                "Citadel.InvocationRequest.allowed_operations must be non-empty strings, got: #{inspect(operation)}"
      end)

    if normalized == [] do
      raise ArgumentError, "Citadel.InvocationRequest.allowed_operations must not be empty"
    end
  ...
  defp validate_allowed_operations!(value) do
    raise ArgumentError,
          "Citadel.InvocationRequest.allowed_operations must be a list of strings, got: #{inspect(value)}"
  end
  ...
  defp validate_authority_packet!(packet) do
    raise ArgumentError,
          "Citadel.InvocationRequest.authority_packet must be AuthorityDecision.v1, got: #{inspect(packet)}"
  end
  ...
  defp validate_boundary_intent!(intent) do
    raise ArgumentError,
          "Citadel.InvocationRequest.boundary_intent must be Citadel.BoundaryIntent, got: #{inspect(intent)}"
  end
  ...
  defp validate_topology_intent!(intent) do
    raise ArgumentError,
          "Citadel.InvocationRequest.topology_intent must be Citadel.TopologyIntent, got: #{inspect(intent)}"
  end
  ...
  defp validate_extensions!(value) do
    ...
    unless is_map(normalized) do
      raise ArgumentError, "Citadel.InvocationRequest.extensions must normalize to a JSON object"
    end
    ...
    if unknown_namespaces != [] do
      raise ArgumentError,
            "Citadel.InvocationRequest.extensions only allows #{@extensions_namespaces |> inspect()} namespaces, got: " <>
    ...
    case Map.get(normalized, "citadel") do
      ...
      nested ->
        raise ArgumentError,
              "Citadel.InvocationRequest.extensions[\"citadel\"] must be a JSON object, got: #{inspect(nested)}"
    end
  end
    ...
    case Map.get(citadel_extensions, "ingress_provenance") do
      ...
      provenance when is_map(provenance) ->
        ...
        if offending_keys != [] do
          raise ArgumentError,
                "Citadel.InvocationRequest.extensions[\"citadel\"][\"ingress_provenance\"] must carry refs or hashes, not raw payload keys: " <>
                  inspect(offending_keys)
        end

      provenance ->
        raise ArgumentError,
              "Citadel.InvocationRequest.extensions[\"citadel\"][\"ingress_provenance\"] must be a JSON object, got: #{inspect(provenance)}"
    end
  end
end
