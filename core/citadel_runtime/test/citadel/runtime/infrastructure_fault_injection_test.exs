...
defmodule Citadel.Runtime.InfrastructureFaultInjectionTest do
  ...
  defmodule TestClock do
    def utc_now do
      Agent.get(Citadel.Runtime.InfrastructureFaultInjectionTest.TestClock, & &1)
    end
    ...
    def advance!(amount, unit) do
      Agent.update(Citadel.Runtime.InfrastructureFaultInjectionTest.TestClock, fn datetime ->
  ...
  setup do
    ...
    if wave12_enabled?() do
      case ToxiproxyHarness.availability_result!("Citadel.Runtime Wave 12 fault injection") do
        :ok -> :ok
        {:skip, _reason} -> :ok
      end
