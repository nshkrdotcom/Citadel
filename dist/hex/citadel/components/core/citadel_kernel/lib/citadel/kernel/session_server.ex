defmodule Citadel.Kernel.SessionServer do
  @moduledoc """
  ...
  alias Citadel.Kernel.Staleness
  ...
  def init(opts) do
    ...
    base_state = %{
      ...
      invocation_supervisor:
        Keyword.get(opts, :invocation_supervisor, Citadel.Kernel.InvocationDispatchSupervisor),
      projection_supervisor:
        Keyword.get(opts, :projection_supervisor, Citadel.Kernel.ProjectionDispatchSupervisor),
      local_supervisor:
        Keyword.get(opts, :local_supervisor, Citadel.Kernel.LocalDispatchSupervisor),
      invocation_handler:
  ...
  defp publish_trace_family(state, family, attrs) do
    ...
    _ = Citadel.Kernel.TracePublisher.publish_trace(state.trace_publisher, envelope)
    :ok
  end
  ...
  defp invariant_failure!(reason) do
    raise RuntimeError, "Citadel.Kernel.SessionServer invariant failure: #{reason}"
  end
